...
{
    $user = Auth::user();
    $donationAmount = $request->amount; // Assuming donation amount comes in the request.
    $user->donations()->create(['amount' => $donationAmount]);

    $totalDonation = $user->donations->sum('amount');
    $eligibleRewards = Reward::where('min_donation_amount', '<=', $totalDonation)->get();
    ...
    foreach ($eligibleRewards as $reward) {
        ...
        if (!$userReward->wasRecentlyCreated) continue;

        $qrCode = QrCode::size(200)->generate($userReward->voucher_code);
        // Save or display QR code
    }

    return redirect()->route('campaign')->with('success', 'Donation and rewards processed.');
}
