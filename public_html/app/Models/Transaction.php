...
class Transaction extends Model
{
    ...
    public function processDonation($userId, $amount) {
    ...
    $rewards = Reward::where('amount_required', '<=', $amount)->get();
    foreach ($rewards as $reward) {
        // Assign reward to user
        $userReward = new UserReward();
        $userReward->user_id = $userId;
        $userReward->reward_id = $reward->id;
        $userReward->voucher_code = strtoupper(uniqid('RW-'));
        $userReward->is_claimed = false;
        $userReward->save();
    }
}
